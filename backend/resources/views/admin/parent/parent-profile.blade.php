    ...
    ***********************************-->
    <div class="content-body">
        <div class="container-fluid">
                ...
                <div class="card shadow-sm">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h3 id="parentName" class="mb-0">--</h3>

                            <button
                                id="createParentLoginBtn"
                                class="btn btn-sm btn-warning px-3"
                                
                            >
                                <i class="fa fa-key me-1"></i> Create / Reset Login
                            </button>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <p><strong>Occupation:</strong><br>
                                    <span id="parentOccupation">--</span>
                                </p>
                            </div>
                            <div class="col-md-3">
                                <p><strong>Status:</strong><br>
                                    <span id="parentStatus" class="badge badge-success">Active</span>
                                </p>
                            </div>
                        </div>
            ...
            <div class="row">
                <div class="col-12">
                    <h4 class="mb-3">Students under this Parent</h4>
                </div>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function ()
    {
        ...
        const createLoginBtn =
            document.getElementById("createParentLoginBtn");

        // ===============================
        // GET PARENT ID FROM URL PATH
        // Example: /admin/parents/profile/41
        // ===============================
        ...
        if (createLoginBtn)
        {
            createLoginBtn.addEventListener("click", function ()
            {
                ...
                createOrResetParentLogin(currentParentId);
            });
        }
        ...
        .done(function (res)
        {
            ...
            // Now load parent profile
            loadParentProfile(currentParentId);
        })
        .fail(function ()
        {
        ...
        // LOAD PARENT PROFILE
        // ===============================
        function loadParentProfile(parentId)
        {
            apiRequest("GET", `/parents/${parentId}`)
            .done(renderParentProfile)
            .fail(function ()
            {
        ...
        // ===============================
        function renderParentProfile(parent)
        {
            ...
            document.getElementById("parentEmail").innerText =
                parent.email ?? "-";

            document.getElementById("parentOccupation").innerText =
                parent.occupation ?? "-";
            ...
            // ===============================
            // Show Create Login button only for Admin
            // ===============================
            const isAdmin =
                currentUserRole === "super_admin" ||
                currentUserRole === "school_admin";

            if (isAdmin && createLoginBtn)
            {
                createLoginBtn.style.display =
                    "inline-block";

                createLoginBtn.disabled =
                    false;
            }
        ...
        // CREATE / RESET LOGIN
        // ===============================
        function createOrResetParentLogin(parentId)
        {
            if (!confirm(
                "Create or reset login credentials for this parent?"
            ))
            {
                return;
            }
            ...
            apiRequest("POST",
                `/parents/${parentId}/create-login`)
            .done(function (res)
            {
                document.getElementById("parentLoginEmail").value =
                    res.email;

                document.getElementById("parentLoginPassword").value =
                    res.password;

                new bootstrap.Modal(
                    document.getElementById("parentLoginModal")
                ).show();
            })
            .fail(function (xhr)
            {
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title">Parent Login Credentials</h5>
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <input id="parentLoginEmail" class="form-control" readonly>
                </div>

                <div class="mb-3">
                    <label class="form-label">Temporary Password</label>
                    <input id="parentLoginPassword" class="form-control" readonly>
                </div>
            </div>
