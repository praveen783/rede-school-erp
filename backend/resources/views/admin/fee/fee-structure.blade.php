        ...
        ***********************************-->
        <div class="content-body">
            <div class="container-fluid">
                ...
                <div class="row page-titles mx-0">
                    <div class="col-sm-6 p-md-0">
                        <div class="welcome-text">
                            ...
                            <span>Create and manage class-wise fee structures</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-xl-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form id="feeStructureForm" class="row">

                                    <div class="col-md-4 mb-3">
                                        <label>Academic Year</label>
                                        <select id="academicYearId" class="form-control" required></select>
                                    </div>

                                    <div class="col-md-4 mb-3">
                                        <label>Class</label>
                                        <select id="classId" class="form-control" required></select>
                                    </div>
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input type="text" id="structureName"
                                            class="form-control"
                                            placeholder="Class 3 – AY 2025-26"
                                            required>
                                    </div>
                ...
                <!-- (Fee Structures List table will come BELOW this later) -->
                ...
                <div class="row mt-4">
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Fee Structures List</h4>
                            </div>
                            <div class="card-body">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadClasses() {
        apiRequest("GET", "/classes")
            .done(response => {
                ...
                const classes = response.data ?? response; //  important line
    ...
    document.getElementById("feeStructureForm")
        ...
        apiRequest("POST", "/fee-structures", payload)
            .done(() => {
            ...
            .fail(err => {
                alert(err.responseJSON?.message ?? "Failed to create fee structure");
            });
    });
                ...
                data.forEach(fs => {
                    tbody.innerHTML += `
                        <tr>
                            <td>${fs.name}</td>
                            <td>${fs.class?.name ?? "-"}</td>
                            <td>${fs.academic_year?.name ?? "-"}</td>
                            <td>
                                <span class="badge ${
                                    fs.is_active ? "bg-success" : "bg-secondary"
                                }">
                                ...
                                ${
                                    fs.is_active
                                        ? `<button class="btn btn-sm btn-info"
                                             onclick="viewStructure(${fs.id})">
                                             View
                                           </button>
                                           <button class="btn btn-sm btn-primary"
                                            onclick="assignStructure(${fs.id})">
                                            Assign Students
                                            </button>`
                                        : `
                                           <button class="btn btn-sm btn-primary"
                                             onclick="configureStructure(${fs.id})">
                                             Configure
                                           </button>
                                           <button class="btn btn-sm btn-success"
                                             onclick="activateStructure(${fs.id})">
                                             Activate
                                           </button>`
                                }
                            </td>
    ...
    ================================= */
    window.activateStructure = function (id) {
        if (!confirm("Activate this fee structure? This action cannot be undone.")) {
            return;
        }

        apiRequest("PATCH", `/fee-structures/${id}/activate`)
            .done(res => {
                alert(res.message);
                loadFeeStructures();
            })
    ...
    ================================= */
    window.configureStructure = function (id) {
        window.location.href = `/admin/fee/fee-structure/${id}/items`;
    };

    window.viewStructure = function (id) {
        window.location.href = `/admin/fee/fee-structure/${id}/view`;
    };

    window.assignStructure = function (id) {
        window.location.href = `/admin/fee/fee-structure/${id}/assign`;
    };
