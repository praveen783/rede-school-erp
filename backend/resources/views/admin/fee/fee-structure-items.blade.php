                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Fee Heads & Amounts</h4>
                            </div>
                            <div class="card-body">

                                <form id="feeItemsForm">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const structureId = pathParts[pathParts.length - 2]; 
    ...
    /* ===============================
       LOAD FEE HEADS + ITEMS
       IMPORTANT:
       - GET /fee-structures/{id}/items DOES NOT EXIST
       - So we ONLY load fee heads
       - Existing items will be loaded AFTER save
    =============================== */
    function loadFeeItems() {
        ...
        apiRequest("GET", `/fee-structures/${structureId}`)
            .done(res => {
                ...
                // Build item map: fee_head_id => amount
                res.items.forEach(item => {
                    itemMap[item.fee_head_id] = item.amount;
                });

                apiRequest("GET", "/fee-heads")
                    .done(heads => {

                        const tbody = document.getElementById("feeItemsTable");
                        tbody.innerHTML = "";

                        heads.forEach(head => {

                            const checked = itemMap[head.id] !== undefined;
                            const amount  = itemMap[head.id] ?? "";
    ...
    document.addEventListener("change", function (e) {
        if (!e.target.classList.contains("fee-check")) return;

        const id = e.target.dataset.feeHeadId;
        const amountInput = document.querySelector(
            `.fee-amount[data-fee-head-id="${id}"]`
        );
    ...
    /* ===============================
       SAVE FEE ITEMS
    =============================== */
    document.getElementById("feeItemsForm")
        .addEventListener("submit", function (e) {

        e.preventDefault();

        if (isActive) {
            alert("Cannot modify an active fee structure");
            return;
        }
        ...
        if (items.length === 0) {
            alert("Select at least one fee head and enter amount");
            return;
        }

        apiRequest("POST", `/fee-structures/${structureId}/items`, { items })
            .done(() => {
                alert("Fee items saved successfully");
                document.getElementById("activateBtn").disabled = false;
            })
            .fail(err => {
                alert(err.responseJSON?.message ?? "Failed to save fee items");
            });
    });
    ...
    document.getElementById("activateBtn")
        ...
        if (!confirm("Activate this fee structure? You cannot edit it later.")) {
            return;
        }

        apiRequest("PATCH", `/fee-structures/${structureId}/activate`)
            .done(res => {
                alert(res.message);
                window.location.href = "{{url('/admin/fee/fee-structure')}}";
            })
            .fail(err => {
                alert(err.responseJSON?.message ?? "Activation failed");
            });
    });
