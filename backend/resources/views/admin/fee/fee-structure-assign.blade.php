...
<meta http-equiv="content-type" content="text/html;charset=UTF-8" />
	<!-- Title -->
	<title>Fee Structure Assign | By Admin</title>
    ...
    .installment-box .box-content h6 {
        color: #ffffff;
        margin-bottom: 5px;
    }

    .installment-box .box-content p {
        font-size: 13px;
        color: #aeb6bf;
        margin: 0;
    }
    ...
    .installment-box input:checked + .box-content {
        font-weight: 600;
        color: #4e73df;
    }
    </style> -->
    ...
    ***********************************-->
    <div class="content-body">
        <div class="container-fluid">
            ...
            <div class="row mb-3">
                <div class="col-12">
                    <h4>Assign Fee Structure</h4>
                    <p id="structureInfo" class="text-muted"></p>
                </div>
            </div>

            <div class="card">
                <div class="card-header">
                    <h5>Installment & Student Selection</h5>
                </div>
                                ...
                                <div class="col-md-3 mb-3">
                                    <label class="installment-box">
                                        <input type="radio"
                                            ...
                                            value="2">
                                        <div class="box-content">
                                            <h6>2 Installments</h6>
                                            <p>Split into 2 equal parts</p>
                                        </div>
                                    </label>
                                </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!structureId) {
        alert("Invalid fee structure");
        window.location.href = "{{ url('/admin/fee/fee-structure') }}";
        return;
    }

    function checkAssignmentStatus() {

        apiRequest("GET", `/fee-structures/${structureId}/assignment-status`)
            .done(res => {
                ...
                if (res.is_assigned) {
                    ...
                    document
                        .querySelectorAll("input[name='installment_option']")
                        .forEach(radio => {
                            radio.disabled = true;

                            if (radio.value == res.installments) {
                                radio.checked = true;
                            }
                        });
                    ...
                    const assignBtn =
                        document.querySelector("#assignFeeForm button[type='submit']");

                    assignBtn.disabled = true;
                    assignBtn.classList.remove("btn-primary");
                    assignBtn.classList.add("btn-secondary");
                    assignBtn.innerText =
                        `Already Assigned (${res.students_count} Students)`;

                    /* Show Banner */
                    if (!document.getElementById("structureAssignedAlert")) {

                        const container =
                            document.querySelector(".container-fluid");

                        container.insertAdjacentHTML("afterbegin", `
                            <div id="structureAssignedAlert"
                                class="alert alert-success alert-dismissible fade show mb-4">
                                <strong>Fee Structure Already Assigned.</strong><br>
                                Students: ${res.students_count}<br>
    ...
    /* ===============================
       LOAD FEE STRUCTURE INFO
    =============================== */
    function loadStructureInfo() {
        apiRequest("GET", `/fee-structures/${structureId}`)
            .done(res => {

                if (res.is_active != 1) {
                    alert("Only ACTIVE fee structures can be assigned");
                    window.location.href = "{{ url('/admin/fee/fee-structure') }}";
                    return;
                }

                selectedClassId = res.class_id;

                document.getElementById("structureInfo").innerText =
                    `${res.name} | ${res.class.name} | ${res.academic_year.name}`;
    ...
    function loadStudents() {
        ...
        apiRequest("GET", `/students?class_id=${selectedClassId}`)
            .done(res => {
    ...
    /* ===============================
       ASSIGN FEE STRUCTURE + INSTALLMENT
    =============================== */
    document.getElementById("assignFeeForm")
        ...
        document.querySelectorAll(".student-check:checked")
            .forEach(cb => studentIds.push(cb.value));
        ...
        const installmentOption = document.querySelector(
            'input[name="installment_option"]:checked'
        );

        const installmentCount = installmentOption
            ? parseInt(installmentOption.value)
            : 1; // fallback safety
        ...
        .fail(err => {
            alert(err.responseJSON?.message ?? "Assignment failed");
        });
    });
