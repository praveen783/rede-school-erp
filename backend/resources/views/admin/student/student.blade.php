        ...
        ***********************************-->
		...
		<div class="form-head d-flex flex-wrap mb-sm-4 mb-3 align-items-center">
			<div class="me-auto  d-lg-block mb-3">
				<h2 class="text-black font-w700 mb-0">Students</h2>
				<!-- <p class="mb-0">Lorem ipsum  dolor sit amet </p> -->
			</div>
			
			<a href="{{ url('/admin/students/create') }}" class="btn btn-primary btn-rounded mb-3"><i class="fa fa-user-plus me-3"></i>New Admission</a>			
		</div>
        <!-- SEARCH INPUT -->
        <div class="row mb-3">
            <div class="col-md-5">
                <input
                    ...
                    id="studentSearch"
                    class="form-control"
                    placeholder="Search by student name, parent name, admission no"
                >
            </div>
            <div class="col-md-2">
					...
					<table class="table table-responsive-lg mb-4 dataTablesCard card-table text-black" id="example5">
						<thead>
							<tr>
                                ...
                                <th style="width: 220px; text-align: center;">Login-Credintials</th>
							</tr>  
						</thead>
   ...
   <!-- Scripts -->
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        window.loadStudents = function (page = 1)
        {
            ...
            apiRequest("GET", "/students", params)
                .done(function(res){
        ...
        function renderStudents(res) {
            ...
            res.data.forEach((student, index) => {
                ...
                const doj = student.date_of_joining
                ...
                tbody.insertAdjacentHTML("beforeend", `
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn btn-sm btn-warning ms-2" onclick="editStudent(${student.id})"> Edit </button>
                        </td>
                        <td class="text-center">
                            ${renderStudentLoginCell(student)}
                        </td>
                    </tr>
                `);
            });
        ...
        function renderStudentLoginCell(student) {
            ...
            if (!student.user_id) {
                return `
                    <button
                        ...
                        onclick="handleStudentLogin(${student.id}, false)">
                        Create Login
                    </button>
                `;
            }
            ...
            // Login exists → Reset
            const createdAt = student.login_created_at
                ? new Date(student.login_created_at).toLocaleString()
                : "—";
            ...
            return `
                <div>
                    <button
                        ...
                        onclick="handleStudentLogin(${student.id}, true)">
                        Reset Password
                    </button>
        ...
        window.handleStudentLogin = function (studentId, isReset) {

            const message = isReset
                ? "Reset login password for this student? The old password will stop working."
            ...
            apiRequest("POST", `/students/${studentId}/create-login`)
                .done(function (res) {

                    document.getElementById("loginUsername").value = res.username;
                    document.getElementById("loginEmail").value = res.system_email;
                    document.getElementById("loginPassword").value = res.password;
                    ...
                    // 🔄 Refresh table to update status + timestamp
    ...
    window.viewStudent = function (studentId) {
        apiRequest("GET", `/students/${studentId}`)
            .done(function (student) {
                ...
                const modal = new bootstrap.Modal(
                    document.getElementById("studentViewModal")
                );
                modal.show();
            })
            .fail(() => alert("Failed to load student details"));
    };
    window.toggleStudentStatus = function (studentId, action) {
        ...
        apiRequest("PATCH", `/students/${studentId}/${action}`)
            .done(function () {
                alert(`Student ${actionText}d successfully`);
                loadStudents(currentPage); // refresh table
            })
            .fail(function () {
                alert(`Failed to ${actionText.toLowerCase()} student`);
            });
    };
    ...
    <div class="modal fade" id="studentViewModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-body" id="studentViewBody">
                <!-- Filled dynamically -->
            </div>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="mb-3">
                <label class="form-label">Login Email (System)</label>
                <input
                    type="text"
                    id="loginEmail"
                    class="form-control"
