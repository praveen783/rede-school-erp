...
<meta http-equiv="content-type" content="text/html;charset=UTF-8" />
	<!-- Title -->
	<title>Owlio Laravel | Student</title>
	...
	<meta property="og:description" content="Owlio Laravel | Student" />
                ...
                <ul class="navbar-nav header-right">
                    ...
                    <li class="nav-item dropdown header-profile">
                        <a class="nav-link" href="#" role="button" data-bs-toggle="dropdown">
                            <!-- <img src="public/images/profile/pic1.jpg" width="20" alt="" class="rounded-circle"> -->
        ...
        <div class="row page-titles mx-0">
            <div class="col-sm-6 p-md-0">
                <div class="welcome-text">
                    <h4>Admit Card Generation</h4>
                    <span>Create admit cards class-wise in one click</span>
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Generate Admit Cards</h5>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-3 mb-3">
                                ...
                                <select id="examSession" class="form-control">
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="number" id="durationMinutes" class="form-control" value="180">
                            </div>
                        </div> -->
                        ...
                        <div class="text-end mt-4">
                            ...
                            <!-- NEW: Schedule Exam Button -->
                            <a
                                id="scheduleExamBtn"
                                href="#"
                                class="btn btn-outline-warning ms-2 d-none">
                        ...
                        <!-- NEW: INFO BOX -->
                        <div id="nextStepInfo" class="alert alert-info mt-3 d-none">
                            <strong>Next step:</strong>
                            Please schedule subject-wise exam dates before publishing admit cards.
                        </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    //     preselectDropdowns(classId, sectionId, examId);
    //     refreshActionButtons(examId, classId, sectionId);
    // }

    document
        .getElementById("classSelect")
        .addEventListener("change", loadSections);

    document
        .getElementById("sectionSelect")
        .addEventListener("change", onSectionChange);
...
/* ============================
   BUTTON STATE CHECK
============================ */
function refreshActionButtons(examId, classId, sectionId) {

    apiRequest(
        "GET",
        `/admin/exams/${examId}/admit-card/status?class_id=${classId}&section_id=${sectionId}`
    )
    .done(res => {

        if (res.admit_generated) {
            enableExamScheduleBtn();
        }

        if (res.schedule_created) {
            enablePublishBtn();
        }
    });
}
...
/* ============================
   LOAD CLASSES
============================ */
function loadClasses(preselect = {}) {
    apiRequest("GET", "/classes").done(res => {
        ...
        if (preselect.classId) {
            el.value = preselect.classId;
            loadSections(preselect);
        }
    });
}
...
/* ============================
   LOAD SECTIONS
============================ */
function loadSections(preselect = {}) {
    ...
    apiRequest("GET", `/sections?class_id=${classId}`)
        .done(res => {
            ...
            if (!Array.isArray(sections)) {
                console.error("Invalid sections response:", res);
                return;
            }
            ...
            // ✅ AUTO-SELECT SECTION (URL FLOW)
            if (preselect.sectionId) {
                sectionSelect.value = preselect.sectionId;

                loadExams(
                    classId,
                    preselect.sectionId,
                    preselect   // pass full context
                );
            }
        })
        .fail(err => {
...
function onSectionChange() {
    ...
    loadExams(classId, sectionId);
}

/* ============================
   LOAD EXAMS (BY CLASS + SECTION)
============================ */
function loadExams(classId, sectionId, preselect = {}) {
    ...
    .done(res => {
        ...
        // ✅ AUTO-SELECT EXAM (RETURN FLOW)
        if (preselect.examId) {
            examSelect.value = preselect.examId;
...
const scheduleRouteTemplate = "{{ route('admin.exams.schedule', ':id') }}";


/* ============================
   GENERATE ADMIT CARDS (BULK)
============================ */
function generateAdmitCards() {
    ...
    const payload = {
        ...
        // exam_session: examSession.value,
        // start_time: startTime.value,
        // end_time: endTime.value,
        // duration_minutes: durationMinutes.value
    };

    apiRequest("POST", `/admin/exams/${examId}/admit-card`, payload)
        .done(res => {

            document.getElementById("statusMsg").innerHTML =
                `<div class="alert alert-success">${res.message}</div>`;

            // SHOW "Schedule Exam Subjects" BUTTON
            const scheduleBtn = document.getElementById("scheduleExamBtn");
            scheduleBtn.classList.remove("d-none");

            scheduleBtn.href =
                scheduleRouteTemplate.replace(':id', examId)
                + `?class_id=${classId}&section_id=${sectionId}`;

            // SHOW NEXT STEP INFO
            document.getElementById("nextStepInfo").classList.remove("d-none");
        })
        .fail(err => {
            document.getElementById("statusMsg").innerHTML =
